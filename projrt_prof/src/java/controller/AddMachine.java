 ...
 * @author a
 */
@WebServlet(name = "AddMachine", urlPatterns = {"/AddMachine"})
public class AddMachine extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(operation.equals("add")){
            ms.create(new Machine(marque, reference, new Date(date), Double.parseDouble(prix)));
        }else if(operation.equals("update")){
            String id = request.getParameter("id");
            Machine m = ms.findById(Integer.parseInt(id));
            m.setMarque(marque);
            m.setPrix(Double.parseDouble(prix));
            m.setDateAchat(new Date(date));
